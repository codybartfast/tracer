...
let ``a matrix multiplied by a tuple`` () =
    let m = matrixOfInts [ [1; 2; 3; 4]
                           [2; 4; 4; 2]
                           [8; 6; 4; 1]
                           [0; 0; 0; 1] ]
    let b = point 1.0 2.0 3.0
    let expected = PrimPoint (point 18.0 24.0 33.0)
    let actual = m |* b
    Assert.Equal (expected, actual)

[<Fact>]
let ``multiplying a matrix by identity matrix`` () =
    let m = matrixOfLists [ [0.0; 1.0; 2.0; 4.0]
                            [1.0; 2.0; 4.0; 8.0]
                            [2.0; 4.0; 8.0; 16.0]
                            [4.0; 8.0; 16.0; 32.0] ]
...
let ``multiplying the identity matrix by a tuple`` () =
    let a = raw 1.0 2.0 3.0 4.0
    let actual = identity () |* a
    Assert.Equal(PrimRaw a, actual)   

[<Fact>]
let ``transposing a matrix`` () =
    let m  = matrixOfLists [ [0; 9; 3; 0]
                             [9; 8; 0; 8]
                             [1; 8; 5; 3]
                             [0; 0; 5; 8] ]
...
let ``calculating the inverse of a matrix`` () =
    ...
    let n = n |> Array2D.map (fun n -> Math.Round(n, 5))
    Assert.Equal(expectedB, n)

[<Fact>]
let ``calculating the inverse of another matrix`` () =
    let m = matrixOfInts [ [8; -5; 9; 2]
                           [7; 5; 6; 1]
                           [-6; 0; 9; 6]
                           [-3; 0; -9; -4] ]
...
let ``multiplying a product by its inverse`` () =
    let m = matrixOfInts [ [ 3; -9;  7;  3]
                           [ 3; -8;  2; -9]
                           [-4;  4;  4;  1]
                           [-6;  5; -1;  1] ]
    let n = matrixOfInts [ [ 8;  2;  2;  2]
                           [ 3; -1;  7;  0]
                           [ 7;  0;  5;  4]
                           [ 6; -2;  0;  5] ]
    let p = m |*| n
