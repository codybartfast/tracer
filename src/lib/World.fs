...
type World(lights: PointLight list, shapes: Shape list) =
    ...
    member w.ShadeHit(comps: Computations, refRemaining: int) : Color =
        w.Lights
            |> List.map (fun light ->
                let surface =
                    lighting
                ...
                if mat.Reflective > 0.0 && mat.Transparency > 0.0 then
    ...
    member w.ColorAt(ray: Ray, refRemaining: int) =
        ...
        match xs |> hit with
        | None -> black
        | Some hit -> w.ShadeHit(prepareComputations hit ray xs, refRemaining)

    member w.ReflectedColor (comps: Computations, refRemaining: int) : Color =
    ...
    member w.RefractedColor (comps: Computations, refRemaining: int) : Color =
        if refRemaining <= 0 then black else
        ...
        let direction =
            comps.Normalv * (nRatio * cosI - cosT) - (comps.Eyev * nRatio)
        let refractedRay = ray comps.UnderPoint direction
        w.ColorAt(refractedRay, refRemaining - 1)
